    ...
    <div className="mt-4">
      ...
      <div className="flex gap-4">
        {paymentOptions.map((option) => (
          <div
            key={option.id}
            className={`border rounded p-2 cursor-pointer flex flex-col items-center transition ${paymentMode === option.id
                ? "border-blue-500 bg-blue-100"
                : "border-gray-300"
              }`}
            onClick={() => setPaymentMode(option.id)}
          >
            <img src={option.img} alt={option.label} className="w-16 h-16" />
